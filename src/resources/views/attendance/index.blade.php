    ...
    <div class="month-list">
        <div class="month-header">
            ...
            <form action="/attendance/list" method="get" class="month-select-form">
                <label class="month">
                    ...
                    <input type="month" name="month" class="month-select-form-input" value="{{ $currentMonth->format('Y-m') }}" onchange="this.form.submit()">
                </label>
            </form>
        ...
        <table class="attendance-logs__table">
            ...
            <tbody>
                ...
                <tr class="attendance-logs__table--row">
                    <td class="attendance-logs__table--content">{{ $day['date']->translatedFormat('m/d(D)') }}</td>
                    <td class="attendance-logs__table--content">{{ optional($day['attendance'])->start_time ? \Carbon\Carbon::parse($day['attendance']->start_time)->format('H:i') : '' }}</td>
                    <td class="attendance-logs__table--content">{{ optional($day['attendance'])->end_time ? \Carbon\Carbon::parse($day['attendance']->end_time)->format('H:i') : '' }}</td>
                    <td class="attendance-logs__table--content">{{ $day['attendance'] ? gmdate('H:i', $day['attendance']->calculateTotalBreakTime() * 60) : '' }}</td>
                    <td class="attendance-logs__table--content">{{ $day['attendance'] ? gmdate('H:i', $day['attendance']->calculateTotalWorkTime() * 60) : '' }}</td>
