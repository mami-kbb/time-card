            ...
            <div class="form-area">
                <div class="form__group">
                    ...
                    <div class="form__group-content">
                        <p class="form__group-content-name">{{ $user->name }}</p>
                    </div>
                </div>
                <div class="form__group">
                    <div class="form__group-title">
                    ...
                    <div class="form__group-content-date">
                        <p>{{ $workDate->translatedFormat('Y年') }}</p>
                        <p>{{ $workDate->translatedFormat('n月j日') }}</p>
                    </div>
                </div>
                ...
                <div class="form__group form__group--break">
                    <div class="form__group-title">
                        <p class="form__label">{{ $index === 0 ? '休憩' : '休憩' . ($index + 1) }}</p>
                    </div>
                    <div class="form__group-content">
                        @if ($isEditable)
                        <div class="form__input-break">
                            <input type="text" class="time-input" name="breaks[{{ $index }}][new_break_start_time]" value="{{ old("breaks.$index.new_break_start_time") ?? optional($break->break_start_time ?? $break->new_break_start_time)->format('H:i') }}">
                            <span>～</span>
                            <input type="text" class="time-input" name="breaks[{{ $index }}][new_break_end_time]" value="{{ old("breaks.$index.new_break_end_time") ?? optional($break->break_end_time ?? $break->new_break_end_time)->format('H:i') }}">
                        </div>
                ...
                <div class="form__group form__group--break">
                    <div class="form__group-title">
                        <p class="form__label">{{ $displayBreaks->count() === 0 ? '休憩' : '休憩' . ($displayBreaks->count() + 1) }}</p>
                    </div>
                    <div class="form__group-content">
                        <div class="form__input-break">
                            <input type="text" inputmode="numeric"
                                pattern="[0-9:]*" class="time-input" name="breaks[{{ $displayBreaks->count() }}][new_break_start_time]" value="{{ old("breaks.{$displayBreaks->count()}.new_break_start_time") }}">
                            <span>～</span>
                            <input type="text" inputmode="numeric"
                                pattern="[0-9:]*" class="time-input" name="breaks[{{ $displayBreaks->count() }}][new_break_end_time]" value="{{ old("breaks.{$displayBreaks->count()}.new_break_end_time") }}">
                        </div>
